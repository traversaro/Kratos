...
namespace Kratos {
	namespace Testing {
		...
		KRATOS_TEST_CASE_IN_SUITE(Checks, KratosCoreFastSuite)
		{
			...
			KRATOS_STATIC_CHECK_IS_FALSE(false);

			KRATOS_STATIC_CHECK_EQUAL(1.0, 1.0);
			KRATOS_STATIC_CHECK_NOT_EQUAL(1.0, 2.0);

			KRATOS_CHECK_C_STRING_EQUAL("Test", "Test");
			KRATOS_CHECK_C_STRING_NOT_EQUAL("Test ", "Test");

			KRATOS_STATIC_CHECK_LESS(1., 2.);
			KRATOS_STATIC_CHECK_LESS_EQUAL(1., 1.);

			KRATOS_STATIC_CHECK_GREATER(2., 1.);
			KRATOS_STATIC_CHECK_GREATER_EQUAL(2., 2.);

			KRATOS_STATIC_CHECK_STRING_CONTAIN_SUB_STRING(std::string("Test"), "es");
		}
