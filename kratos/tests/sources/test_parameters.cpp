...
namespace Kratos {
	namespace Testing {
		...
		KRATOS_TEST_CASE_IN_SUITE(ParametersIterator, KratosCoreFastSuite)
		{
			...
			KRATOS_STATIC_CHECK_EQUAL(i_parameter.name(), std::string("int_value"));
			KRATOS_STATIC_CHECK(i_parameter->IsInt());
			KRATOS_STATIC_CHECK_EQUAL(i_parameter->GetInt(), 10);
			KRATOS_STATIC_CHECK((++i_parameter)->IsDouble());
			KRATOS_STATIC_CHECK_EQUAL((i_parameter++)->GetDouble(), 2.0);
			KRATOS_STATIC_CHECK((*i_parameter).IsBool());
			KRATOS_STATIC_CHECK_EQUAL((*i_parameter).GetBool(), true);
			...
			KRATOS_STATIC_CHECK_EQUAL(size, 5);
        ...
        {
            Parameters p;
            KRATOS_STATIC_CHECK_EQUAL(p.WriteJsonString(), "{}");
        }
        ...
        KRATOS_TEST_CASE_IN_SUITE(Parameters_Swap1, KratosCoreFastSuite)
        {
            ...
            p1.swap(p2);
            KRATOS_STATIC_CHECK(p1.Has("bar"));
            KRATOS_STATIC_CHECK(p1["bar"].GetBool() == false);
            KRATOS_STATIC_CHECK(p2.Has("foo"));
            KRATOS_STATIC_CHECK(p2["foo"].GetDouble() == 1.0);
        }
        ...
        KRATOS_TEST_CASE_IN_SUITE(Parameters_MoveConstructor1, KratosCoreFastSuite)
        {
            ...
            KRATOS_STATIC_CHECK_EQUAL(p1.WriteJsonString(), "{}");
            KRATOS_STATIC_CHECK_EQUAL(p2.WriteJsonString(), R"({"foo":{"bar":1.0}})");
        }
