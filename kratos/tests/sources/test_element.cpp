    ...
    namespace Testing {
        ...
        */
        KRATOS_TEST_CASE_IN_SUITE(ElementCloneOperator, KratosCoreFastSuite)
        {
            ...
            KRATOS_STATIC_CHECK_EQUAL(p_clone_of_elem->Id(), 2);
            KRATOS_STATIC_CHECK_DOUBLE_EQUAL(p_clone_of_elem->GetValue(DISTANCE), 12.1);
            KRATOS_STATIC_CHECK_DOUBLE_EQUAL(p_clone_of_elem->GetValue(VELOCITY_X), 32.4);
            KRATOS_STATIC_CHECK_DOUBLE_EQUAL(p_clone_of_elem->GetValue(VELOCITY_Y), 0.00);
            KRATOS_STATIC_CHECK_DOUBLE_EQUAL(p_clone_of_elem->GetValue(VELOCITY_Z), 0.00);
            KRATOS_STATIC_CHECK(p_clone_of_elem->Is(ACTIVE));
        }
    }  // namespace Testing.
}  // namespace Kratos.
