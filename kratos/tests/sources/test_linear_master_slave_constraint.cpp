...
KRATOS_TEST_CASE_IN_SUITE(LinearMasterSlaveConstraintTests, KratosCoreFastSuite)
{
        ...
        KRATOS_STATIC_CHECK_EQUAL(master_vector.size(), 1);
        KRATOS_STATIC_CHECK_EQUAL(slave_vector.size(), 1);

        KRATOS_STATIC_CHECK_EQUAL(transformation_matrix.size1(), 1);
        KRATOS_STATIC_CHECK_EQUAL(transformation_matrix.size2(), 1);
        KRATOS_STATIC_CHECK_EQUAL(constant_vector.size(), 1);

        KRATOS_STATIC_CHECK_DOUBLE_EQUAL(transformation_matrix(0,0), 1.0); // TODO: Check -> comparison between two doubles ??
        KRATOS_STATIC_CHECK_DOUBLE_EQUAL(constant_vector(0), 0.0);
}
