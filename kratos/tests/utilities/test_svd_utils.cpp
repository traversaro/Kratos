...
namespace Kratos
{
    namespace Testing
    {
        ...
        KRATOS_TEST_CASE_IN_SUITE(SVDUtilsjacobiSVD2x2Test, KratosCoreFastSuite)
        {
            ...
            for (std::size_t i = 0; i < 2; ++i) {
                for (std::size_t j = i; j < 2; ++j) {
                    KRATOS_STATIC_CHECK_NEAR(auxmat22(i,j), a_matrix(i,j), tolerance);
                }
            }

            // Check SV are correct (value and order)
            KRATOS_STATIC_CHECK_NEAR(s_matrix(0,0), 1.053846, tolerance);
            KRATOS_STATIC_CHECK_NEAR(s_matrix(1,1), 0.023021, tolerance);
        }
        ...
        KRATOS_TEST_CASE_IN_SUITE(SVDUtilsJacobiSVD3x3Test, KratosCoreFastSuite)
        {
            ...
            for (std::size_t i = 0; i < 3; ++i) {
                for (std::size_t j = i; j < 3; ++j) {
                    KRATOS_STATIC_CHECK_NEAR(auxmat33(i,j), a_matrix(i,j), tolerance);
                }
            }

            // Check SV are correct (value and order)
            KRATOS_STATIC_CHECK_NEAR(s_matrix(0,0), 1.554701, tolerance);
            KRATOS_STATIC_CHECK_NEAR(s_matrix(1,1), 0.412674, tolerance);
            KRATOS_STATIC_CHECK_NEAR(s_matrix(2,2), 0.059198, tolerance);
        }
        ...
        KRATOS_TEST_CASE_IN_SUITE(SVDUtilsConditionNumberTest, KratosCoreFastSuite)
        {
            ...
            KRATOS_STATIC_CHECK_NEAR(condition_number, 26.2607, tolerance);
        }
