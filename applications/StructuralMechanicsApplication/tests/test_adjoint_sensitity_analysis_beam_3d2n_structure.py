...
class TestAdjointSensitivityAnalysisBeamStructure(KratosUnittest.TestCase):
    ...
    def test_local_stress_response(self):
        ...
        with controlledExecutionScope(_get_test_working_dir()):
            ...
            for element_id in element_list:
                sensitivities_to_check.append(model_primal.GetModelPart(model_part_name).Elements[element_id].GetValue(I22_SENSITIVITY))
    ...
    def nodal_displacement_response(self):
        # solve primal problem
        with controlledExecutionScope(_get_test_working_dir()):
            with open("beam_test_parameters.json",'r') as parameter_file:
                ProjectParametersPrimal = Parameters( parameter_file.read())
    ...
    def strain_energy_response(self):
        # solve primal problem
        with controlledExecutionScope(_get_test_working_dir()):
            with open("beam_test_parameters.json",'r') as parameter_file:
                ProjectParametersPrimal = Parameters( parameter_file.read())
        ...
        with controlledExecutionScope(_get_test_working_dir()):
            ...
            adjoint_analysis = structural_mechanics_analysis.StructuralMechanicsAnalysis(model_primal, ProjectParametersAdjoint)
