...
*/
class TriGenPFEMModelerVMS : public TriGenPFEMModeler
{
public:
    ///@name Type Definitions
    ///@{
...
//qua inizia la parte cambiata

                if ( preserved_list1[el] == false && number_of_structure_nodes == 1 )
                {
                    int noFSEdge = 0;
                    int Vertexiter[4] = {0,1,2,0};
                    for (int iVertex = 0 ; iVertex < 3 ; iVertex++ )
                    {
                        if (    ( temp[Vertexiter[iVertex]].FastGetSolutionStepValue(IS_FREE_SURFACE) && temp[Vertexiter[iVertex + 1]].FastGetSolutionStepValue(IS_FREE_SURFACE) )
                             || ( temp[Vertexiter[iVertex]].FastGetSolutionStepValue(IS_FREE_SURFACE) && temp[Vertexiter[iVertex + 1]].FastGetSolutionStepValue(IS_STRUCTURE) )
                             || ( temp[Vertexiter[iVertex]].FastGetSolutionStepValue(IS_STRUCTURE) && temp[Vertexiter[iVertex + 1]].FastGetSolutionStepValue(IS_FREE_SURFACE) )
                           )
                        {
                            noFSEdge++;
                        }
                    ...
                    if (noFSEdge == 3)
                    {
                        double bigger_alpha = my_alpha*1.23;
                        if( AlphaShape(bigger_alpha,temp) && number_of_structure_nodes!=3)
                        {
    // 						  	if(nboundary==3 && number_of_structure_nodes > 1 && nfs > 0) //if it is = 3 pressure problems -> do not add it
    // 							{
...
//calculate average h
    ...
    /// Assignment operator.
    TriGenPFEMModelerVMS & operator=(TriGenPFEMModelerVMS const& rOther);
