    ...
    Vector& rPrincipalStrain, unsigned int& region, Vector& rPrincipalStressUpdated)
{
    ...
    Vector unknown_vector, delta_unknown_vector;
    Vector rhs_vector = ZeroVector(3);
    ...
    mStateFunctionFirstDerivative  = ZeroVector(3);
    mStateFunctionSecondDerivative = ZeroVector(6);
    Vector principal_stress_vector = rPrincipalStress;
...
void BoundingSurfacePlasticFlowRule::CalculateLHSMatrix(Matrix& rLHSMatrix, const Vector& rPrincipalStressVector, const Vector& rUnknownVector, const double& rK_p)
{
    // Reset Zero
...
void BoundingSurfacePlasticFlowRule::CalculateHessianMatrix_2x2(Matrix& rHessianMatrix)
{
    // Material parameters
...
void BoundingSurfacePlasticFlowRule::ComputeElasticMatrix_2X2(const Vector& rPrincipalStressVector, const double& rVolumetricStrain, const double& rDeviatoricStrain, Matrix& rElasticMatrix)
{
    // Material parameters
    const double swelling_slope = mpYieldCriterion->GetHardeningLaw().GetProperties()[SWELLING_SLOPE];
...
void BoundingSurfacePlasticFlowRule::ComputePlasticMatrix_2X2(const Vector& rPrincipalStressVector, const double& rVolumetricStrain, const double& rDeviatoricStrain, const Matrix& rElasticMatrix, Matrix& rPlasticMatrix)
{
    ...
    // Compute matrix_G
    Matrix matrix_G = prod(hessian_matrix_H, rElasticMatrix); 
...
void BoundingSurfacePlasticFlowRule::CalculateMeanStress(const double& rVolumetricStrain, const double& rDeviatoricStrain, double& rMeanStress)
{
    const double swelling_slope = mpYieldCriterion->GetHardeningLaw().GetProperties()[SWELLING_SLOPE];
    const double alpha_shear    = mpYieldCriterion->GetHardeningLaw().GetProperties()[ALPHA_SHEAR];
...
void BoundingSurfacePlasticFlowRule::CalculateDeviatoricStress(const double& rVolumetricStrain, const Vector & rDeviatoricStrainVector, Vector& rDeviatoricStress)
{
    double ref_pressure = mpYieldCriterion->GetHardeningLaw().GetProperties()[PRE_CONSOLIDATION_STRESS];
    const double ocr = mpYieldCriterion->GetHardeningLaw().GetProperties()[OVER_CONSOLIDATION_RATIO];
    ref_pressure /= ocr;    
...
void BoundingSurfacePlasticFlowRule::ComputeElastoPlasticTangentMatrix(const RadialReturnVariables& rReturnMappingVariables, const Matrix& rNewElasticLeftCauchyGreen, const double& alfa, Matrix& rConsistMatrix)
{
    ...
    Matrix plastic_matrix_D_p = IdentityMatrix(2);
    ...
    Matrix matrix_D_ep = prod(elastic_matrix_D_e, plastic_matrix_D_p);
...
void BoundingSurfacePlasticFlowRule::UpdateStateVariables(const Vector rPrincipalStress, const double rAlpha, const double rConsistencyParameter)
{
