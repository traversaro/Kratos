...
#============================================================================================================================
	def SolveSolutionStep(self):
		...
		# if self.FEM_Solution.step == 84:
		# 	self.FEM_Solution.GraphicalOutputPrintOutput() # borrar
		# 	Wait()
	...
	def GenerateDEM(self): # 3D version
		...
		for Element in FEM_elements:
			...
			if is_active == False and DEM_Generated == False: # Let's generate the remaining DEM of this FE
				...
				# print("elemento eliminado: " , Element.Id)
				# lista = [8281,8285,8883,8884,10804,10806,11401,11402]

				# for i in range(0, len(lista)):
				# 	if Element.Id == lista[i]:
				# 		print("elemento eliminado en gen: " , Element.Id)
				# 		Wait()
				print("elemento eliminado: " , Element.Id)
			...
			# Case with all the DEM generated by the surrounding Elems
				...
				# print("cosa rara: ",Element.Id)

				# print("hay DEM en sus nodos: ",
				#  Element.GetNodes()[0].GetValue(KratosFemDem.IS_DEM),Element.GetNodes()[1].GetValue(KratosFemDem.IS_DEM),
				#  Element.GetNodes()[2].GetValue(KratosFemDem.IS_DEM),Element.GetNodes()[3].GetValue(KratosFemDem.IS_DEM))
	...
	def CheckForPossibleIndentations(self): # Verifies if an element has indentations between its DEM
		...
		for Element in FEM_elements:
			...
			if NumberOfDEM > 1 and is_active == True and DEM_Generated == False:  # Case in which the DEM have been generated by its neighbours
				...
				# print("elemento eliminado en indent: " , Element.Id)
				# lista = [8281,8285,8883,8884,10804,10806,11401,11402]

				# for i in range(0, len(lista)):
				# 	if Element.Id == lista[i]:
				# 		print("elemento eliminado en avoid: " , Element.Id)
				# 		Wait()
...
#============================================================================================================================
	def CheckInactiveNodes(self):
		...
		for Element in FEM_Elements:
			...
			# 	for i in range(0, len(lista)):
			# 		if Element.Id == lista[i]:
			# 			print("el elem esta aqui",Element.Id)
			# 			Wait()
		...
		for node in FEM_Nodes:
			...
			if NumberOfActiveElements == 0 and node.GetValue(KratosFemDem.INACTIVE_NODE) == False:
				...
				# if self.FEM_Solution.step == 84 and node.Id == 1448:
				# 	print("nodo inactivo: ", Id)
				# 	print(NumberOfActiveElements)
				# 	Wait()
