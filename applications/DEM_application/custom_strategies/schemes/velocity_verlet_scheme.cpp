...
//         if(verbose) KRATOS_INFO("DEM") << "Assigning VelocityVerletScheme to properties " << pProp->Id() << std::endl;
...
//         if(verbose) KRATOS_INFO("DEM") << "Assigning VelocityVerletScheme to properties " << pProp->Id() << std::endl;
    ...
    void VelocityVerletScheme::CalculateNewRotationalVariablesOfRigidBodyElements(
                int StepFlag,
                Node < 3 >& i,
