...
for normalvar in range(2):
    ...
    for dim, nnodes in zip(dim_combinations, nnodes_combinations):
        ...
        PreviousTangentSlip = DefineMatrix('PreviousTangentSlip', nnodes, dim)
        ...
        for node in range(nnodes):
            ...
            for idim in range(dim):
                TangentSlip[node,idim] = PreviousTangentSlip[node,idim] + auxTangentSlip * TangentSlave[node, idim]
